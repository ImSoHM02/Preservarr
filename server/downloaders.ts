// Download client types (internal to this module, not DB types)
...
export interface DownloadFile {
  ...
  size: number;
  progress: number;
  priority: "skip" | "normal" | "high" | "maximum" | "low" | "off";
  wanted?: boolean;
}
  ...
  static async getAllDownloads(downloader: Downloader): Promise<DownloadStatus[]> {
    ...
    if (downloader.category) {
      ...
      return downloads.filter((t) => {
        ...
        if (t.category) {
          return String(t.category).toLowerCase() === filterCategory;
        }
